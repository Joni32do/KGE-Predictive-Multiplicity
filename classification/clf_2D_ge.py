...
def example_baseline_and_epsilon_set():
    h0 = Custom_SVM(w=np.array([1, 0]), b=0)
    eps_set = []
    for i in range(2):
        clf = Custom_SVM(w=np.array([np.cos((2*i-1)*np.pi/20), np.sin((2*i-1)*np.pi/20)]), b=0)
        eps_set.append(clf)
    return h0, eps_set
...
def main():
    ...
    # Shade area
    axs[0].fill_between([-1, 0], [-1, -1], [1, 1], color=FALSE_RED, alpha=0.4)
    axs[0].fill_between([0, 1], [-1, -1], [1, 1], color=TRUE_GREEN, alpha=0.4)
    ...
    for clf, color in zip(eps_set, ["tab:blue", "tab:orange", "tab:green"]):
